...
public class UserController {
	 ...
	 * {
	 * 
	 * //여기서 POST 방식으로 key=value 데이터를 요청 (카카오쪽으로) //HttpsURLConnection url; //OkHttp
	 * //Retrofit2(라이브러리) - 안드로이드 //RestTemplate
	 * 
	 ...
	 * 
	 * //HttpHeader와 HttpBody를 하나의 오브젝트에 담기 //exchange라는 함수가 httpEntity라는 오브젝트를 받기로
	 * 되어있음 HttpEntity<MultiValueMap<String, String>> kakaoTokenRequest = new
	 * HttpEntity<>(params,headers);
	 * 
	 * //Http 요청하기 - Post 방식으로 - 그리고 response 변수의 응답 받음. ResponseEntity<String>
	 * response = rt.exchange( "https://kauth.kakao.com/oauth/token",
	 * HttpMethod.POST, kakaoTokenRequest, String.class );
	 * 
	 * //응답받은 데이터를 오브젝트에 쏙 넣음 //Gson, Json Simple, ObjectMapper ObjectMapper
	 * objectMapper = new ObjectMapper(); OAuthToken oaAuthToken = null; try {
	 ...
	 * 
	 * //HttpHeader와 HttpBody를 하나의 오브젝트에 담기 //exchange라는 함수가 httpEntity라는 오브젝트를 받기로
	 * 되어있음 HttpEntity<MultiValueMap<String, String>> kakaoProfileRequest = new
	 * HttpEntity<>(headers2);
	 * 
	 * //Http 요청하기 - Post 방식으로 - 그리고 response 변수의 응답 받음. ResponseEntity<String>
	 ...
	 * 
	 * //응답받은 데이터를 오브젝트에 쏙 넣음 //Gson, Json Simple, ObjectMapper ObjectMapper
	 * objectMapper2 = new ObjectMapper(); KakaoProfile kakaoProfile = null; try {
	 ...
	 * ) .email(kakaoProfile.getKakao_account().getEmail()) .password(wcKey)
	 * .name("이름") .address("입력필요") .phone("입력필요") .gender("성별") .role("ROLE_USER")
	 * .build();
	 * 
	 * //이미 가입된 사람인지 비가입자인지 체크를 해서 처리 User originUser =
	 * userService.회원찾기(kakaoUser.getUsername()); System.out.println(originUser);
	 * if(originUser.getUsername()==null) { System.out.println("기존회원아님");
	 * userService.회원가입(kakaoUser); } System.out.println("자동 로그인");
	 * 
	 * Authentication authentication = authenticationManager .authenticate(new
	 * UsernamePasswordAuthenticationToken(kakaoUser.getUsername(), wcKey));
